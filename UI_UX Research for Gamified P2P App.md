...
| **Transaction Flow** | Interactive commitment sheets, pre-filled values, and slider controls 19 | Quick-reply prompts; completed in 4 to 5 clicks maximum 12 |
...
In paid peer-to-peer DAREs, trust is directly tied to financial legibility.1 The design of the transaction interface must ensure the user has complete clarity on when their money is locked, how it is partitioned, and the exact steps required for release.34
...
In a Skill-Based DARE where both participants contribute equal stakes, the payout calculation must clearly display the deduction of the platform escrow rake:

```text
settlement_payout = (stake_amount * 2) - platform_fee
```
...
* The net amount the eligible Skill-Based winner or Task-Based performer will receive upon challenge resolution.1
...
For live challenge spectator surfaces, the platform can borrow clarity patterns from prediction-market interfaces without becoming a prediction market.45 The core UI design principle is to prioritize result clarity over complex data dumps.45
Active event cards must show clear, simple resolution criteria above the fold, providing users with the exact parameters that dictate the accepted result, refund, or dispute path.45
Post-MVP social mechanics can be enhanced by integrating "Replicate DARE" flows.1 Rather than forcing a spectator to navigate away from the live feed and rebuild a challenge manually, the feed can overlay an option to copy a successful DARE format.47
The user journey for replicating a DARE requires a simple, three-step interaction:

1. **Spot a DARE format** on a public creator's profile card, complete with verified performance metrics such as completion rate, dispute rate, win rate, and historically tracked results.48
2. **Tap the "Replicate" icon**, which loads the challenge constitution, category, proof method, and resolution mode into a pre-filled creation sheet.21
3. **Adjust the stake or reward** and tap "Confirm" to create a new DARE instance under current escrow rules.48

To maintain trust, the UI must display a clear verification badge next to any user whose historical DARE results are pulled from live backend ledger and settlement records, distinguishing verified challenge creators from unverified profiles.48  All user interactions in the Live Court must write asynchronously to backend tables.1

Database writes:

* `court_chat` for spectator comments.
* `dare_votes` for spectator live voting.
* `dare_quiz_answers` for answer_key score tracking.

By separating chat messages, votes, and answer_key inputs from transaction writes, the UI maintains high responsiveness and protects financial ledger entries from system bottlenecks.1
...
DARE implements a decentralized adjudication workflow inspired by the game-theoretic principles of Kleros.1 When a participant files a dispute, required participants submit their evidence through a secure upload portal.1 Jurors are drawn from eligible community members with KYC, trust, and anti-collusion checks; token-staked jury selection is not part of the MVP.50
...
The dispute flow keeps escrow funds locked, collects evidence from required participants, routes the case into juror voting, tallies consensus, and then settles the escrow according to the final verdict and DARE type.
...
The platform must enforce the strict transaction limits of the Money Laundering Act, 2022, which restricts individual cash transfers outside financial institutions to ₦5,000,000 and corporate transactions to ₦10,000,000.1 To maintain compliance, the payment interface must programmatically limit single deposits, stakes, and rewards below these limits, while requiring enhanced KYC and source-of-wealth validations if transaction volumes approach these values.1
...
* **Deposit and Loss Limits:** Users must be able to configure spending boundaries for daily, weekly, or monthly intervals.53 To prevent impulsive paid participation, lowering a spending limit must execute immediately, while raising or removing a limit must enforce a mandatory 24-hour cooling-off delay.55
...
CREATE TABLE dares (
    ...
    player_b_id UUID REFERENCES profiles(id), -- Nullable for open Skill-Based or Task-Based DAREs
    title VARCHAR(100) NOT NULL,
    constitution TEXT NOT NULL,
    type VARCHAR(20) CHECK (type IN ('answer_key', 'witnessed', 'evidence')),
...
* **Purpose:** Logs individual player answers for answer_key challenge calculations.
* **Data Structure:**
```sql
