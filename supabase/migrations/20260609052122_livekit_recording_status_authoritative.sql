  ...
  on conflict on constraint live_court_rooms_dare_id_key do update
    set status = case
        ...
        recording_status = case
          when public.live_court_rooms.recording_status in ('recording', 'available', 'processing', 'failed', 'disabled') then public.live_court_rooms.recording_status
          else 'not_started'
        end,
        started_at = coalesce(public.live_court_rooms.started_at, excluded.started_at),
        recording_started_at = public.live_court_rooms.recording_started_at,
        updated_at = now()
