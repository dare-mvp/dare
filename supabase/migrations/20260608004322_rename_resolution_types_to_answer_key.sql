update public.dares
set resolution_type = case resolution_type
  when concat('evid', 'enced') then 'evidence'
  when concat('algo', 'rithmic') then 'answer_key'
  when concat('hon', 'our') then 'witnessed'
  else resolution_type
end
where resolution_type in (concat('evid', 'enced'), concat('algo', 'rithmic'), concat('hon', 'our'));

alter table public.dares
  drop constraint if exists dares_resolution_type_valid;

alter table public.dares
  add constraint dares_resolution_type_valid
