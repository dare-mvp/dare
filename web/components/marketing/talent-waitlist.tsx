...
import { joinTalentWaitlist, type TalentJoinState } from '@/app/(marketing)/talent-actions';
import { CheckCircle2, Copy, Check, Lock, Clock, ArrowDown } from 'lucide-react';
import { TALENT_CHALLENGE_CAP, TALENT_CHALLENGE_END_LABEL, TALENT_CHALLENGE_START_LABEL } from '@/lib/talent-challenge-config';
import { trackEvent } from '@/lib/analytics';

const initialState: TalentJoinState = {};

const SHARE_TEXT = 'I just joined the Show Me Your Talent Dare Challenge 🎯 Complete 8 tasks and earn ₦5,000 to your DARE wallet. Sign up with my link:';
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: 'Show Me Your Talent Dare Challenge — DARE App',
          text: SHARE_TEXT,
          url: referralLink,
        });
        trackEvent('talent_task_share_click', { source: 'step1', method: 'native' });
        return;
      } catch {
        // User cancelled — fall through to WhatsApp
      }
    }
    trackEvent('talent_task_share_click', { source: 'step1', method: 'whatsapp' });
    window.open(
...
export function TalentWaitlist({
  referredBy,
  spotsRemaining,
  isFull,
  isExpired,
  ...
  const [state, formAction, pending] = useActionState(joinTalentWaitlist, initialState);
  const [copied, setCopied] = useState(false);

  useEffect(() => {
    if (state.ok && state.referralCode && state.referralUrl) {
      onSuccess?.(state.referralCode, state.referralUrl);
      trackEvent('talent_waitlist_join', {
  ...
  async function handleCopy() {
    ...
    try {
      ...
      trackEvent('talent_referral_link_copied', { source: 'step1' });
    } catch {
      setCopied(false);
    }
  }
      ...
      <div className="flex gap-4 rounded-2xl border border-white/8 bg-brand-surface p-5 sm:p-6">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Opens on{' '}
            <span className="text-foreground font-medium">{TALENT_CHALLENGE_START_LABEL}</span>.
            Check back then.
          </p>
        </div>
      </div>
  ...
  if (isFull || state.error === 'challenge_full') {
    return (
      <div className="flex gap-4 rounded-2xl border border-white/8 bg-brand-surface p-5 sm:p-6">
        ...
        <div>
          <p className="font-semibold text-foreground text-sm">All {TALENT_CHALLENGE_CAP} spots are filled</p>
          <p className="mt-1 text-sm text-muted-foreground">
            Follow{' '}
            <a
    ...
    return (
      <div className="flex gap-4 rounded-2xl border border-white/8 bg-brand-surface p-5 sm:p-6">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Ended {TALENT_CHALLENGE_END_LABEL}. Follow{' '}
            <a
              href="https://www.instagram.com/dareappofficial"
              target="_blank"
              rel="noopener noreferrer"
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            {state.isDuplicate ? (
              ...
              <>
                <p className="text-sm font-semibold text-foreground">You&apos;re in. Task 01 is done.</p>
                <p className="text-xs text-muted-foreground">
                  Your referral link is ready. Scroll down to see all 8 tasks.
                </p>
              </>
            )}
          </div>
    ...
    <div className="space-y-4 rounded-2xl border border-white/8 bg-brand-surface p-5 sm:p-6">
      <div className="space-y-1.5">
        <div className="flex flex-wrap justify-between gap-1 font-mono text-[10px] text-muted-foreground">
          <span>{spotsRemaining} of {TALENT_CHALLENGE_CAP} spots remaining</span>
          <span>Closes {TALENT_CHALLENGE_END_LABEL}</span>
        </div>
        <div className="flex gap-0.5" aria-hidden>
          {Array.from({ length: 20 }, (_, i) => {
            const filledBlocks = Math.round(
              ((TALENT_CHALLENGE_CAP - spotsRemaining) / TALENT_CHALLENGE_CAP) * 20,
            );
            return (
              <div
                key={i}
      ...
      <p className="text-sm text-muted-foreground">
        Enter your email to get your unique referral link and unlock all 8 tasks.
      </p>
